...
#include "nav2_navfn_planner/test_navfn_planner.hpp"
...
test_NavfnPlanner::test_NavfnPlanner()
...
test_NavfnPlanner::activate()
{
  ...
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(&test_NavfnPlanner::dynamicParametersCallback, this, _1));
}

void
test_NavfnPlanner::deactivate()
{
  RCLCPP_INFO(
...
nav_msgs::msg::Path test_NavfnPlanner::createPlan(
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  // Create a PoseStamped object for the goal
  geometry_msgs::msg::PoseStamped goal_pose;
  goal_pose.header = path.header;
  goal_pose.pose = goal.pose;  // Set the goal pose
  ...
  // Return the path with only the goal point as the waypoint
  return path;
}

bool
test_NavfnPlanner::isPlannerOutOfDate()
{
  if (!planner_.get() ||
...
void
test_NavfnPlanner::smoothApproachToGoal(
...
bool
test_NavfnPlanner::getPlanFromPotential(
  const geometry_msgs::msg::Pose & goal,
  nav_msgs::msg::Path & plan)
{
  // clear the plan, just in case
...
double
test_NavfnPlanner::getPointPotential(const geometry_msgs::msg::Point & world_point)
{
  unsigned int mx, my;
...
bool
test_NavfnPlanner::worldToMap(double wx, double wy, unsigned int & mx, unsigned int & my)
{
...
void
test_NavfnPlanner::mapToWorld(double mx, double my, double & wx, double & wy)
{
...
void
test_NavfnPlanner::clearRobotCell(unsigned int mx, unsigned int my)
{
...
test_NavfnPlanner::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters)
{
...
PLUGINLIB_EXPORT_CLASS(nav2_navfn_planner::test_NavfnPlanner, nav2_core::GlobalPlanner)
